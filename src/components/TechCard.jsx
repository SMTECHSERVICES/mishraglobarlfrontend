...
const TechCard = ({ heading, techList }) => {
  ...
  return (
    <motion.div
      ...
      whileHover={{ 
        scale: 1.03,
        boxShadow: "0 20px 25px -5px rgba(245, 196, 45, 0.2), 0 10px 10px -5px rgba(245, 196, 45, 0.1)"
      }}
      onHoverStart={() => setIsHovered(true)}
      onHoverEnd={() => setIsHovered(false)}
    >
      ...
      <motion.div 
        className="absolute inset-0 z-0"
        initial={{ background: "linear-gradient(135deg, #fff9db 0%, #ffe98a 100%)" }}
        animate={{
          background: isHovered 
            ? "linear-gradient(135deg, #fff176 0%, #ffd54f 100%)" 
            : "linear-gradient(135deg, #fff9db 0%, #ffe98a 100%)"
        }}
        transition={{ duration: 0.4 }}
      />
      ...
      <motion.div 
        ...
        transition={{ duration: 0.3 }}
        style={{
          background: "radial-gradient(600px circle at center, rgba(255, 223, 70, 0.4) 0%, transparent 80%)"
        }}
      />
      ...
      <div className="relative z-10 p-6 min-h-[250px] flex flex-col">
        ...
        <motion.div 
          className="absolute top-4 right-4 text-5xl font-bold text-yellow-100"
          animate={{ 
            opacity: isHovered ? 0.8 : 0.3,
            y: isHovered ? -5 : 0
          }}
          transition={{ duration: 0.3 }}
        >
          {/* Optional Floating Icon or Badge */}
        </motion.div>
